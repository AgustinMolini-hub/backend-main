...
describe('Orders API', function () {
    ...
    describe('GET /api/orders', function () {
        ...
        it('debe paginar correctamente los pedidos', async function () {
        ...
        it('debe limitar a 100 la cantidad máxima de pedidos por página', async function () {

            const response =
                await request(app)
                    .get('/api/orders?page=1&limit=500');
    ...
    describe('POST /api/orders', function () {
        ...
        it('debe crear un pedido correctamente con datos válidos', async function () {
            ...
            expect(orderInDatabase)
                .to.not.equal(null);


            expect(orderInDatabase.user.toString())
                .to.equal(
                    testUser._id.toString()
                );
    ...
    describe('GET /api/orders/:id', function () {
        ...
        it('debe devolver un pedido existente por ID', async function () {
            ...
            expect(responseOrder._id.toString())
                .to.equal(
                    order._id.toString()
                );
            ...
            expect(responseOrder.user.toString())
                .to.equal(
                    testUser._id.toString()
                );
    ...
    describe('POST /api/orders/:id/receipt', function () {
        ...
        it('debe subir correctamente un comprobante PDF', async function () {
            ...
            expect(response.body.payload)
                .to.have.property('receipt');


            expect(response.body.payload.receipt)
                .to.have.property(
                    'originalName',
                    'comprobante-test.pdf'
                );
            ...
            expect(orderInDatabase.receipt)
                .to.exist;


            expect(orderInDatabase.receipt.originalName)
                .to.equal(
                    'comprobante-test.pdf'
                );


            expect(orderInDatabase.receipt.mimetype)
                .to.equal(
                    'application/pdf'
                );
        ...
        it('debe devolver 404 cuando el pedido no existe', async function () {
            ...
            const response =
                await request(app)
                    .post(
                        `/api/orders/${fakeOrderId}/receipt`
                    )
                    .attach(
                        'file',
                        Buffer.from(
                            '%PDF-1.4 comprobante inexistente'
                        ),
                        {
                            filename:
                                'comprobante-inexistente.pdf',
                            contentType:
                                'application/pdf'
                        }
                    );
        ...
        it('debe devolver 400 cuando el ID del pedido es inválido', async function () {
            ...
            const response =
                await request(app)
                    .post(
                        '/api/orders/id-invalido/receipt'
                    )
                    .attach(
                        'file',
                        Buffer.from(
                            '%PDF-1.4 comprobante ID invalido'
                        ),
                        {
                            filename:
                                'comprobante-id-invalido.pdf',
                            contentType:
                                'application/pdf'
                        }
                    );
            ...
            const response =
                await request(app)
                    ...
                    .attach(
                        'document',
                        Buffer.from(
                            '%PDF-1.4 campo incorrecto'
                        ),
                        {
                            filename:
                                'comprobante-campo-invalido.pdf',
                            contentType:
                                'application/pdf'
                        }
                    );
